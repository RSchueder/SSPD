...
from substances_26 import CAS_26
...
# IN THE DISCTIONARY (MANUALLY ADDED) THAT TELLS THE PROGRAM HOW TO
...
# requested parameters, those in the model
paramReq = c.fetchall()
c.execute("SELECT STREAM_EU_parameter FROM STREAM_EU_database_dictionary")
# names of possessed parameters, those in the database
paramPos = c.fetchall()
...
method, modelspec1, modelspec2, modelspec3, ref_temp = STREAM_EU_search_list(paramReq,paramReq)
headers = 0
...
###############################################################################
    ...
    write_STREAM_EU_output(CASreq, allCAS, paramReq, paramPos, search_t, ii, model, method, modelspec1, modelspec2,
